...
// Serve only the public directory, not the entire project root
...
app.use(apiLimiter); // Apply general rate limiting to all routes below
...
app.get('/api/audit-logs', authenticateToken, requireAdmin, async (req, res, next) => {
  try {
...
app.post('/api/audit-logs', authenticateToken, async (req, res, next) => {
  try {
    await pool.execute(
      `INSERT INTO audit_logs
       (action, entity_type, entity_id, actor_id, actor_name, actor_role, details)
...
app.put('/api/users/:id/status', authenticateToken, requireAdmin, async (req, res, next) => {
  try {
    ...
    const [result] = await pool.execute(
      ...
      [status, req.user.id, status, req.params.id]
    );
...
app.post('/api/users/:id/admin-request', authenticateToken, async (req, res, next) => {
  try {
...
app.put('/api/admin-requests/:id/status', authenticateToken, requireAdmin, async (req, res, next) => {
...
app.put('/api/users/:id/role', authenticateToken, requireAdmin, async (req, res, next) => {
  try {
...
app.put('/api/users/:id/password', authenticateToken, requireAdmin, async (req, res, next) => {
  try {
    const newPassword = String(req.body.newPassword || '');
...
app.get('/api/surveys', authenticateToken, async (req, res, next) => {
  try {
...
app.get('/api/surveys/:id', authenticateToken, async (req, res, next) => {
  try {
...
app.put('/api/surveys/bulk', authenticateToken, requireAdmin, async (req, res, next) => {
...
app.post('/api/surveys', authenticateToken, async (req, res, next) => {
  try {
...
app.delete('/api/surveys/:id', authenticateToken, requireAdmin, async (req, res, next) => {
  try {
